...
class rival(pygame.sprite.Sprite):
    ...
    def __init__(self,startx,starty ,endx, endy): 
        ...
        self.y = starty
        self.image = pygame.transform.scale(REST, (int(REST.get_width()), int(REST.get_height())))
        ...
        self.pathx = [self.rect.centerx, (self.startx + self.width) , (self.endx - self.width) ]                 #path taken by rival
        self.pathy = [self.rect.centery, (self.starty + self.height), (self.endy - self.height)]
    ...
    def draw(self):                                                   #fn called in maindraw()
        ...
        if self.COUNT > 15:                                           #no. of images in list = 16, => max index = 15
            ...
            screen.blit(left, self.rect.center)                                     #display change on screen
            ...
            screen.blit(right, self.rect.center)
            self.COUNT += 1
            self.dir = 'r'
            ...
            screen.blit(up, self.rect.center)
            self.COUNT += 1
            self.dir = 'u'
            ...
            screen.blit(down, self.rect.center)
            self.COUNT += 1
            self.dir = 'd'
          ...
          #turns by theta angle
            screen.blit(shoot, self.rect.center)                                #displayed on screen
        ...
        if self.move:
            if self.starty == self.endy:                                  #if y co-ordinates are same => rival moves along x-axis
                ...
                if self.velx > 0:                                               #if rival is moving forward (towards right)
                    if self.rect.centerx + self.velx < self.pathx[2]:        #check if character has reached the end
                        self.rect.centerx += self.velx                                     #if no, move right with velx speed
                    else:                                                       #if yes, make velx negative so it enters next condn.
                        self.velx = self.velx * -1
                        
                else :                                                          #rival is moving backwards
                    if self.rect.centerx- self.velx >  self.pathx[1]:                      #check if character has reached the beginning
                        self.rect.centerx += self.velx                                     #if no, move left with velx speed(move right with -velx speed)
                ...
                if self.vely > 0:
                    if self.rect.centery + self.vely < self.pathy[2]:
                    ...
                    if self.rect.centery - self.vely >  self.pathy[1]:
                        self.rect.centery += self.vely
                    else:
                        self.vely = self.vely * -1    

        pygame.draw.rect(screen, (255,0,0), pygame.Rect(self.rect.centerx, self.rect.centery, 128, 128),  2)
        rival.checkPoint(self, lad.x, lad.y, self.rect.centerx,self.rect.centery , self.dir)     #checks if player is in rival's field
